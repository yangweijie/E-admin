...
class SidebarGrid extends Component
{
     ...
     * @return $this
     */
    public function width($left = 5,$right = 19){
    ...
    public function __construct(Model $model, $grid, $id, $name, $children)
    {
        ...
        if($this->grid){
            $this->grid->bindAttValue('initLoad', false, true);
            $this->sidebar->bindAttr('initLoad', $this->grid->bindAttr('initLoad'), true);
    ...
    public function jsonSerialize()
    {
        ...
        if(is_null($this->grid)){
            ...
            $row->column($this->grid, $this->column[1] ?? 19);
            $this->content($row);
        }
        return $row; // TODO: Change the autogenerated stub
    }
}
