...
class Grid extends Component
{
    ...
    public function column($field = '',$label = '')
    {
        if($field instanceof \Closure){
            ...
            $column = new Column('group'.md5($label.time()), $label, $this);
    ...
    protected function getColumnAttribute(){
        foreach ($this->column as $key=>$column){
            if($column->attr('children_row')){
                unset($this->column[$key]);
            }
        }
        return array_column($this->column, 'attribute');
    }
    public function jsonSerialize()
    {
        ...
        if (request()->has('ajax_request_data') && request()->get('eadmin_class') == $this->callClass && !$this->attr('static')) {
            ...
            $this->attr('columns', $this->getColumnAttribute());
